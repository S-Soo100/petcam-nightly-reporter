...
# basking(휴식) 추가(2026-07-16): 휴식은 비정보성이라 moving/unseen 처럼 자동 하이라이트 제외.
# 환경변수 override 동작은 그대로 — 명시 env 로 기본 목록을 덮어쓸 수 있다.
REGISTER_SKIP_ACTIONS = frozenset(
    os.environ.get("REGISTER_SKIP_ACTIONS", "moving,basking,error,unseen,shedding").split(",")
)
